...
public class SplitterUtil {
    ...
    public static List<PuzzlePiece> split(Bitmap bitmap, int level, int dishWidth, int dishHeight)
            throws FileNotFoundException {
//        Bitmap[] bmcover = DishManager.getBitmapCover();
        List<PuzzlePiece> pieces = new ArrayList<>(level * level);
        ...
        for (int i = 0; i < level; i++) {
            for (int j = 0; j < level; j++) {
                if(i == 0) {
                    if (j == 0) {
                        k = PuzzleManager.COVER_TOP_LEFT;
                    } else if(j == level - 1) {
                        k = PuzzleManager.COVER_TOP_RIGHT;
                    }
                    else  {
                        k = PuzzleManager.COVER_TOP;
                    }
                }
                else if(i == level - 1) {
                    if(j == 0) {
                        k = PuzzleManager.COVER_BOTTOM_LEFT;
                    } else if (j == level - 1) {
                        k = PuzzleManager.COVER_BOTTOM_RIGHT;
                    } else{
                        k = PuzzleManager.COVER_BOTTOM;
                    }
                }
                else if(j == 0) {
                    k = PuzzleManager.COVER_LEFT;
                }else if(j == level - 1) {
                    k = PuzzleManager.COVER_RIGHT;
                }else  {
                    k = PuzzleManager.COVER_CENTER;
                }
...
//                yValue = i * pieceHeight;
                ...
                Bitmap cover = PuzzleManager.getCover(k);
