...
public class PuzzleManager {
    ...
    private WeakReference<PuzzlleApplication> weakApplication;

    private static final String TAG = "puzzle";

    public static PuzzleManager getInstance() {
        if (mInstance == null) {
            return new PuzzleManager((PuzzlleApplication) PuzzlleApplication.mContext);
        }
        return mInstance;
    }
    ...
    private PuzzleManager(PuzzlleApplication application) {
        weakApplication = new WeakReference<PuzzlleApplication>(application);
    }

    private PuzzleManager(PuzzlleApplication application, int level) {
        weakApplication = new WeakReference<PuzzlleApplication>(application);
        updateLevel(level);
    }
     ...
     *
     * @param picture     新游戏图片
     * @param puzzleBoard 新游戏的拼盘
     */
    public void initNewGame(Bitmap picture, ImageView puzzleBoard) {
        if (picture == null || puzzleBoard == null) return;

        if (mBitmap != null && !mBitmap.isRecycled()) {
            mBitmap.recycle();
        }

        mImageView = puzzleBoard;
        mBitmap = picture;
        mLeftSize = mSize;
        ...
        puzzleBoard.setOnDragListener(new View.OnDragListener() {
            @Override
...
//    }
    ...
    private void initMask() {
        ...
        rectWidth = DensityUtil.dip2px(weakApplication.get().getApplicationContext(), (float) DISH_WIDTH / mLevel);
        rectHeight = DensityUtil.dip2px(weakApplication.get().getApplicationContext(), (float) DISH_HEIGHT / mLevel);
        r = rectWidth / 4;
...
//        canvas.save();
    ...
    private Bitmap getMask() {
        int width = DensityUtil.dip2px(weakApplication.get().getApplicationContext(), (float) DISH_WIDTH);
        int height = DensityUtil.dip2px(weakApplication.get().getApplicationContext(), (float) DISH_HEIGHT);
